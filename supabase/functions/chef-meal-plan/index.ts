...
const FUNCTION_VERSION = "2026-07-23.multi-dish-menu.22";
...
function namedRecipeItemPrompt({
  language,
  resolution,
  meal,
  planningProfile,
  ...
  resolution: DishResolution;
  meal: string;
  planningProfile: Record<string, unknown>;
  pantry: PantryItem[];
  recentMeals: RecentMeal[];
  source: Record<string, unknown> | null;
}) {
  ...
  return `You are Chef Jarvis. Prompt contract version: ${PROMPT_VERSION}. Create exactly one complete recipe for the single named dish in ${language === "zh-TW" ? "Traditional Chinese" : "English"}. Never merge it with another dish. Respect every allergy, dislike, and dietary preference; never recommend an allergen. Prefer pantry items only when they fit the exact dish. ${providerRecipeContext}

${requestPromptEnvelope}

${contextPromptEnvelope}

Recipe accuracy is mandatory:
- Preserve the canonical dish identity, defining ingredients, and defining techniques.
- List every ingredient separately, including oil, water, salt, spices, sauces, and garnishes.
- Each ingredient name must identify one purchasable product and exact form or cut. usda_query must be the same ingredient's specific English USDA search name.
- Never combine products in one ingredient entry. Every quantity must be one exact positive number.
- unit must be exactly one of: g, kg, ml, L, tsp, tbsp, cup, piece, clove, slice, can, pack.
- preparation must be precise, or "no preparation". category must be exactly one of: protein, produce, grain, dairy, seasoning, oil, other.
- Every ingredient used in the steps must appear in ingredients.
- Include at least one directly usable substitution. substitution.from must exactly equal an ingredients[].name, and the replacement must contain its own quantity, unit, preparation, category, and affected step_updates.
- Every material heat or waiting action must state one exact duration and include a matching timer. Never use duration ranges. Untimed actions use timers: [].
- timer kind must be one of: preheat, cook, bake, simmer, boil, steam, rest, marinate, chill, proof, cool.
...
async function generateNamedRecipeItem({
  ...
  let failureStage = "dish_resolution";
  let failureReason = "recipe_schema";
  let failureOutcome: "generation_timeout" | "generation_validation_failed" =
    "generation_validation_failed";
  const recordFailure = (stage: string, reason: string) => {
    const preferred = preferNamedFailureDiagnostic(
      { stage: failureStage, reason: failureReason },
      { stage, reason },
    );
    failureStage = preferred.stage;
    failureReason = preferred.reason;
  };
  const unavailable = () => ({
    requested_dish: meal,
    status: "unavailable",
  ...
  try {
    ...
    if (resolution.requestType === "named_dish" && apiKey) {
      try {
        ...
        resolution = normalizeDishResolution(
          meal,
          candidate,
        ) as unknown as DishResolution;
      } catch (error) {
        recordFailure("dish_resolution", recipeValidationReasonCode(error));
      }
    }
    if (resolution.needsClarification) {
      return {
        ...
        message: resolution.needsDescription
          ? language === "zh-TW"
            ? "請補充可辨識的食材、風格或作法。"
            : "Please add recognizable ingredients, style, or preparation details."
          : undefined,
      };
    }
    if (!apiKey) {
      recordFailure("model_request", "provider_unavailable");
      return unavailable();
    }

    const dishKey = await dishObservabilityKey(resolution.canonicalName);
    let source: Record<string, unknown> | null = null;
    if (resolution.requestType === "named_dish") {
      failureStage = "provider_lookup";
      const providerResult = await fetchTheMealDbRecipe({
        apiKey: theMealDbKey,
        resolution,
        timeoutMs: deadlineTimeout(deadlineAt, 4_000, REFUND_RESERVE_MS),
    ...
    const prompt = namedRecipeItemPrompt({
      language,
      resolution,
      meal,
      planningProfile,
    ...
    const modelAttempts = [
      { name: "gemini-3.1-flash-lite", timeoutMs: 12_000 },
      { name: "gemini-3.5-flash", timeoutMs: 18_000 },
    ];
    for (const [attemptIndex, { name: model, timeoutMs }] of
      ...
      try {
        ...
        let rawText = await requestGeminiRecipe(
          ...
          deadlineTimeout(deadlineAt, timeoutMs, REFUND_RESERVE_MS),
        );
        ...
        for (
          ...
          try {
            ...
            const disposition = recipeValidationDisposition(error);
            recordFailure(failureStage, recipeValidationReasonCode(error));
            if (
              attemptIndex !== 0 ||
        ...
        const plan = resolution.requestType === "named_dish"
          ? {
            ...validatedPlan,
        ...
        mealPlanResponseAllergenGate({ plan }, profile);
        console.log("chef_menu_item_completed", {
          request_id: requestId,
          dish_key: dishKey,
          outcome: "ready",
          model,
        ...
        recordFailure(failureStage, recipeValidationReasonCode(error));
        if (namedFailureOutcome(error) === "generation_timeout") {
          failureOutcome = "generation_timeout";
        }
        console.log("chef_menu_item_generation", {
          request_id: requestId,
          dish: meal.slice(0, 80),
          model,
          outcome: "failed",
          attempt: attemptIndex + 1,
        });
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const menuRequest = parseMenuRequest(meal);
    if (menuRequest.kind === "invalid") {
      return safeRespond(
        {
          error: menuRequest.code === "menu_too_large"
            ? language === "zh-TW"
              ? "一次最多可規劃 6 道料理。"
              : "You can plan up to 6 dishes at a time."
            : language === "zh-TW"
              ? "每一道料理名稱最多 160 個字元。"
              : "Each dish name can contain up to 160 characters.",
          code: menuRequest.code,
        },
        400,
      );
    }
    ...
    if (menuRequest.kind === "menu") {
      if (apiKey) {
        ...
        if (!allowance?.allowed) {
          return safeRespond(
            {
              error: allowance?.reason === "daily_limit"
                ? language === "zh-TW"
                  ? "你今天的餐點規劃額度已用完，請稍後再試。"
            ...
            429,
            { "Retry-After": String(allowance?.retry_after_seconds || 60) },
          );
        }
      }
      ...
      const menuBody = buildMenuResponse(
        menuRequest.originalRequest,
        recipes,
        {
          request_id: requestId,
      ...
      const readyCount = Number(menuBody.meta.ready_count || 0);
      const clarificationCount = Number(
        menuBody.meta.clarification_count || 0,
      );
      if (readyCount === 0 && admin && quotaRequestId && quotaUserId) {
        const refundRequestId = quotaRequestId;
        quotaRequestId = null;
        await refundQuotaSafely(
          admin,
      ...
      console.log("chef_meal_plan_completed", {
        request_id: requestId,
        prompt_version: PROMPT_VERSION,
        outcome: readyCount > 0
          ? readyCount === recipes.length
            ? "menu_ready"
            : "menu_partial"
          : clarificationCount > 0
            ? "menu_clarification_required"
            : "menu_unavailable",
        requested_count: recipes.length,
        ready_count: readyCount,
        duration_ms: Date.now() - requestStartedAt,
      });
      return safeRespond(menuBody, readyCount > 0 || clarificationCount > 0 ? 200 : 503);
    }
